...
export const QuizDisplay = ({ src }) => {
  ...
  //   return await axios.post(https://22d0-34-16-230-37.ngrok-free.app/process,{
  //       'keyid': "java",
  //       "name":"sarthak"
  //   })
  // }, {
  ...
  const removeUn = () => {
    for (let i = 0; i < questionsArray.length; i++) {
      let setarr = new Set();
      questionsArray[i].options.map((option) => {
        setarr.add(option.text);
      });
      let arri = Array.from(setarr);
      questionsArray[i].arr = arri;
    }
  };
  removeUn();
  return (
